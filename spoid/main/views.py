...
class GetComponentListWithFavorite(APIView):
    def post(self, request):
        ...
        for item in sql_data:
            ...
            if item['Price']:
                try:
                    item['LowestPrice'] = min([int(price) for price in item['Price'] if (str.isdigit(price) and int(price) != 0)])
                except:
                    item['LowestPrice'] = 0
                item['LowestShop'] = item['Shop'][item['Price'].index(str(item['LowestPrice']))] if item['LowestPrice'] else 0
                item['LowestURL'] = item['URL'][item['Price'].index(str(item['LowestPrice']))] if item['LowestPrice'] else 0
        ...
        for component in components:
            ...
            query = f"""
                        ...
                        daily_prices_with_shop AS (
                            SELECT p.ComponentID, p.Type, p.Date, p.Price AS MinPrice, p.Shop
                            FROM Price p
                            INNER JOIN daily_min_prices dmp ON p.ComponentID = dmp.ComponentID AND p.Type = dmp.Type AND DATE_FORMAT(p.Date, '%y-%m-d') = DATE_FORMAT(dmp.Date, '%y-%m-%d') AND p.Price = dmp.MinPrice
                        )
                        SELECT
                            dpws.ComponentID,
                            dpws.Type,
            ...
            print((datetime.today() - timedelta(days=2)).strftime('%Y-%m-%d'))
            print((datetime.today() - timedelta(days=3)).strftime('%Y-%m-%d'))
            print((datetime.today() - timedelta(days=4)).strftime('%Y-%m-%d'))
            print((datetime.today() - timedelta(days=5)).strftime('%Y-%m-%d'))
            print((datetime.today() - timedelta(days=6)).strftime('%Y-%m-%d'))
            print((datetime.today() - timedelta(days=7)).strftime('%Y-%m-%d'))
            logger.info(query)
            cursor.execute(query)
            logger.info(cursor)
